...
const COMMANDS = MODULES.filter((m) => m.id !== "footer").map((m) => ({ n: m.n, name: m.name, meta: m.meta, to: m.id }));
...
(function commandPalette() {
  ...
  function jumpTo(cmd) { close(); scrollToId(cmd.to); pushStatus(`JUMP ${cmd.n} · ${cmd.name}`, "ok"); }
